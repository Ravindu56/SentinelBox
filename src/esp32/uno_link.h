...
namespace UnoLink {
    ...
    // Outbound
    void            sendGPS();
    void            sendAck();              // sends "ACK,<millis>" to ATmega
    void            sendRaw(const char *line); // sends arbitrary line
    ...
    // Power coordination (SLEEP / PWR frames from ATmega)
    bool            sleepRequested();
    void            clearSleepRequest();
    const char*     lastPwrState();         // last "PWR,x" value
    void            clearPwrState();
}
